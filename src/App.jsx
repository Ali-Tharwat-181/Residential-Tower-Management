    ...
    try {
      ...
      printElement.innerHTML = `
        <div style="direction: rtl; font-family: Arial, sans-serif; padding: 20px;">
          ...
          <p style="text-align: right; margin-bottom: 20px;">تاريخ التصدير: ${new Date().toLocaleDateString("ar-EG")}</p>
            ...
            <tbody>
              ${residents.map((resident, index) => `
                <tr style="background-color: ${index % 2 === 0 ? '#f9f9f9' : 'white'};">
                  ...
                  <td style="border: 1px solid #ddd; padding: 8px; text-align: right; font-weight: bold;">${resident.name}</td>
                  <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">${resident.floor}</td>
                  <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">${resident.apartment}</td>
                  <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">${resident.phone}</td>
                  <td style="border: 1px solid #ddd; padding: 8px; text-align: right;">${resident.notes || "-"}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
          <p style="text-align: center; font-weight: bold;">  إجمالي عدد السكان  ${residents.length}</p>
        </div>
      `;
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-blue-500 hover:shadow-xl transition-all duration-300">
            <div className="flex items-center">
              ...
              <div className="mr-4">
                ...
                <p className="text-3xl font-bold text-gray-800">{residents.length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-purple-500 hover:shadow-xl transition-all duration-300">
            <div className="flex items-center">
              ...
              <div className="mr-4">
                ...
                <p className="text-lg font-semibold text-gray-800">{new Date().toLocaleDateString('ar-EG')}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="p-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="form-control">
                  <label className="label">
                    <span className="label-text font-semibold text-gray-700">الاسم الكامل</span>
                  </label>
                  <input
                    type="text"
                    placeholder="أدخل اسم الساكن"
                    className={`input input-bordered w-full transition-all duration-300 ${
                      errors.name ? "input-error border-2" : "focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
                    }`}
                    {...register("name")}
                  />
                  {errors.name && (
                ...
                <div className="form-control">
                  <label className="label">
                    <span className="label-text font-semibold text-gray-700">رقم الهاتف</span>
                  </label>
                  <input
                    type="tel"
                    placeholder="010XXXXXXXX"
                    className={`input input-bordered w-full transition-all duration-300 ${
                      errors.phone ? "input-error border-2" : "focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
                    }`}
                    {...register("phone")}
                  />
                  {errors.phone && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="form-control">
                  <label className="label">
                    <span className="label-text font-semibold text-gray-700">الدور</span>
                  </label>
                  <select
                    className={`select select-bordered w-full transition-all duration-300 ${
                      errors.floor ? "select-error border-2" : "focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
                    }`}
                    {...register("floor")}
                  >
                ...
                <div className="form-control">
                  <label className="label">
                    <span className="label-text font-semibold text-gray-700">رقم الشقة</span>
                  </label>
                  <input
                    type="number"
                    placeholder="رقم الشقة"
                    className={`input input-bordered w-full transition-all duration-300 ${
                      errors.apartment ? "input-error border-2" : "focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
                    }`}
                ...
                <div className="form-control">
                  <label className="label">
                    <span className="label-text font-semibold text-gray-700">ملاحظات</span>
                  </label>
                  <input
                    type="text"
          ...
          <div className="p-6">
            {residents.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-gray-600 mb-2">لا يوجد سكان مسجلين بعد</h3>
                <p className="text-gray-500">ابدأ بإضافة ساكن جديد باستخدام النموذج أعلاه</p>
              </div>
            ) : (
                ...
                <table className="table table-zebra w-full" ref={tableRef}>
                  <thead>
                    <tr className="bg-gray-50">
                      ...
                      <th className="text-right font-bold text-gray-700">الاسم</th>
                      <th className="text-center font-bold text-gray-700">الدور</th>
                      <th className="text-center font-bold text-gray-700">الشقة</th>
                      <th className="text-center font-bold text-gray-700">الهاتف</th>
                      <th className="text-right font-bold text-gray-700">ملاحظات</th>
                      <th className="text-center font-bold text-gray-700">الإجراءات</th>
                    </tr>
                  </thead>
                  <tbody>
                    {residents.map((resident, index) => (
                      <tr key={index} className="hover:bg-blue-50 transition-colors duration-200">
                        <td className="text-center font-semibold text-gray-600">{index + 1}</td>
                        <td className="font-bold text-gray-800 text-right">{resident.name}</td>
                        <td className="text-center">
                          <span className="badge badge-primary badge-outline">{resident.floor}</span>
                        </td>
                        <td className="text-center font-semibold text-gray-700">{resident.apartment}</td>
                        <td className="text-center">
                          <span className="font-mono text-blue-600">{resident.phone}</span>
                        </td>
                        <td className="text-right text-gray-600">{resident.notes || "-"}</td>
                        <td>
